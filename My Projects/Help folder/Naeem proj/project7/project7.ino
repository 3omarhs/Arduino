...
//Ultrasonic ultrasonic(D6, D5); //(tigger, echo)  >> echo>D5
int distance;
#include "DHTesp.h"
#define DHTpin 14
DHTesp dht;

  int relay_ground_humidity = D1;
  int relay_LDR = 13;
  int relay_temp = D0;
  int relay_pump = D2;
  int buzzer = D4;
...
//  }
  ...
  for(int i=0; i<=2; i++){   //make i<=2
    if(i == 0){
      ...
      int val = analogRead(Sensor_read);
//      Serial.println(analogRead(Sensor_read));
    }
    else if(i == 1){
      ...
      sprintf (RemoteXY.LDR_val, "%s is %d", str, myvar);
      Serial.print("myvar  ");
...
//      char str[] = "Value";
      int myvar = dht.getTemperature();
//      sprintf (RemoteXY.temp_val, "%s is %d", str, myvar);
      RemoteXY.temp_val = myvar;
    }
      char str[] = "Value";
      sprintf (RemoteXY.LDR_val, "%s is %d", str, val);
    delay(1000);
  }
